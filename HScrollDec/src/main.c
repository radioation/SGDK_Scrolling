...
int main(u16 hard)
{
	...
	while (TRUE)
	{
		s16 sPlaneOffsetA = scrollPlane(BG_A, bg_image_a.tilemap, indexA, FIX32(0.2), &offsetA, &imageOffsetA, &lastSrcColA, &lastDstColA);
		KLog_S2("count: ", count, " offset: ", sPlaneOffsetA);
		KLog_F1("        offsetA: ", offsetA);
		s16 sPlaneOffsetB = scrollPlane(BG_B, bg_image_b.tilemap, indexB, FIX32(1.0), &offsetB, &imageOffsetB, &lastSrcColB,  &lastDstColB);
